...
```mermaid
flowchart LR
    classDef cam     fill:#1f6feb,stroke:#0d4da2,color:#fff
    classDef remote  fill:#8957e5,stroke:#6f42c1,color:#fff
    classDef data    fill:#d97706,stroke:#b45309,color:#fff
    classDef decide  fill:#e3b341,stroke:#bb8009,color:#1f2328
    classDef robot   fill:#238636,stroke:#196c2e,color:#fff
    classDef grip    fill:#da3633,stroke:#b62324,color:#fff

    subgraph PERC["Perception &amp; Grasp Generation"]
      direction LR
      CAM["RealSense<br/>RGB-D 640x480"]:::cam
      ARU["ArUco board<br/>PnP → world TF"]:::cam
      SRC{"Depth source?"}:::decide
      NAT["Native RealSense depth"]:::cam
      FFS["FFS Server<br/>(Fast Foundation Stereo)"]:::remote
      FRM["Aligned RGB + Depth"]:::data
      PROMPT["User prompt<br/>(point or box)"]:::data
      SAM["SAM Server<br/>(Segment Anything)"]:::remote
      MK["Segmentation mask"]:::data
      PC["Deproject masked depth<br/>→ 3D pointcloud (downsampled)"]:::data
      MODEL["Grasp Model Server"]:::remote
      GRASP["Grasp pairs + scores (grasps.npz)"]:::data
      ...
      MK --> PC
      PC -- "POST pcd" --> MODEL
      MODEL -- "grasp pairs" --> GRASP
    end

    subgraph EXEC["Robot Execution"]
      direction LR
      PLAN["Plan: model → board → base frames<br/>(nearest-arm assignment)"]:::data
      CHK{"Table clearance?"}:::decide
      DROP["discard grasp"]:::data
      JOG["Jog to pre-grasp<br/>(manual mode)"]:::robot
      PRIM["Grasp primitive<br/>(threaded move_both)"]:::robot
      A7["xArm7 (blue)"]:::robot
      A6["xArm6 (red)"]:::robot
      GL["Gripper L"]:::grip
      GR["Gripper R"]:::grip
      LIFT["Lift 100 mm"]:::robot
      REL["Lower &amp; release"]:::robot
...
<sub>Legend: blue = camera/hardware, purple = remote server, orange = data/step, yellow = decision, green = robot arm, red = gripper</sub>
<!-- 
## Project Structure
